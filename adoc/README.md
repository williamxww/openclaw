...
## webchat `/chat` 消息从入口到 LLM 的完整调用链

调试 webchat 时一个常见的误区是「以为 `/chat` 走的是 `agent-command.ts`」。实际不是。
`agent-command.ts` 是 `agent` 子命令(CLI / `agent.run` RPC / OpenAI 兼容 HTTP)的入口；
**webchat 走的是另一条路径**，断点要打在对应位置才会命中。
...
走哪条分支由 `resolveAgentHarnessPolicy()` 根据当前 agent / 模型配置决定。
本仓库默认 webchat + 自建 OpenAI 兼容模型走的是 **embedded** 路径。
