...
flowchart TB
    ...
    %% 回复投递
    PI -- "reply payload" --> ROUTE
    ROUTE -- "outbound.send()" --> CH_PLUGIN --> MSGR
    ROUTE -- "WS chat/agent events\n流式推送" --> WSS
    ...
    class DISPATCH,ROUTE,AG_CMD,AG_INT,PI,TOOLS,CTX,MEM,SKILLS coreBox
...
| 核心运行时 | 绿 | `dispatchInboundMessage` → `agentCommandFromIngress` → Pi 主循环（含工具执行、上下文压缩、记忆读写）|
...
Gateway RPC 层
    ↓
agentCommandFromIngress()          ← src/agents/agent-command.ts:1791
    ↓
agentCommand()                     ← src/agents/agent-command.ts:1763
    ↓
agentCommandInternal()             ← src/agents/agent-command.ts:526
    ├─ 解析 Session / 模型 / Auth
    ├─ 构建 Skills Snapshot
