...
装配分两个阶段：**阶段 A 生成蓝图文件**（装配 Agent 负责，到写入 MinIO 为止），**阶段 B 实例化 OPT pod**（xsystem 负责，调其他团队的服务挂载）。
...
用户在 Web 界面填写 OPT 配置
  ...
  ▼
xsystem 组装 OPT JSON
  │ 经 agent 对话通道下发：msg.send
  │   { method: "msg.send",
  │     params: { conv_id, agent_id: "opt-assembler", content: "<OPT 配置>" } }
  ▼
装配 Agent 收到消息（预置“生成 markdown”SKILL，无需插件 / 无专用接口）
  │
  ├─ 步骤 1：读取并校验配置（必填字段检查）
  │
  ├─ 步骤 2：DAG 校验（如有）
  ...
  │
  ├─ 步骤 4：SKILL 直接把文件写入 MinIO（workspace 真源）
  │
  └─ 步骤 5：把结果作为 agent 回复回流给 xsystem
              ✅ 成功：MinIO 蓝图路径 + agent 列表 + 文件清单
              ❌ 失败：失败步骤 + 原因 + 修复建议
              （装配到此结束，不创建 / 不挂载 pod）
  ▼
xsystem 收到回流 → 把文件清单 / 版本写入 MySQL
```
...
| 装配请求怎么发给 claw | 复用现成的 agent 对话通道（`AgentProtocolService.msg.send`），把 OPT 配置作为消息内容发给预置 SKILL 的装配 Agent | 无需给 claw 装插件、不新增专用接口；装配等同于和一个特定 agent 对话 |
| 生成的 markdown 怎么落 MinIO | 装配 Agent 的预置 SKILL 内部直接写 MinIO；agent 只回流文件清单 | 大文件不经对话通道（受 4096 字符限制），xsystem 只记元数据 |
