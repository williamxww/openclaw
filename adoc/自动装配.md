...
### 6.2 两类定时任务：蓝图任务 vs 用户任务

定时任务有两个来源，混在一个 `jobs.json` 里会在重启时互相覆盖，必须拆开：

| | 蓝图任务（设计态） | 用户任务（运行态） |
|---|---|---|
| 谁创建 | 设计器统一配置，进 MinIO 蓝图 | 用户在自己的 OPT 里临时加 |
| 作用范围 | 一套蓝图下**所有** pod 共享 | **仅当前 pod**，不外泄 |
| 真源 | MinIO（`cron/jobs.blueprint.json`） | pod 本地（`cron/jobs.user.json`） |
| 能否回写 MinIO | 是（设计器改→同步所有 pod） | **绝不**，否则污染蓝图、泄漏给他人 pod |

**关键约束**：一套设计态蓝图会实例化成张三、李四…多个 pod。用户任务若回写 MinIO，会进入共享蓝图，导致张三加的任务跑到李四的 pod 上。所以用户任务**只能留在本 pod 的私有盘**，永不上行。

### 6.3 两个文件，pod 内合并

放弃"整个 `jobs.json` 当蓝图同步"的做法，改为**两个独立来源文件 + 启动时合并成 openclaw 真正读取的 `jobs.json`**：

| 文件 | 内容 | 归属 | 重启行为 |
|------|------|------|---------|
| `cron/jobs.blueprint.json` | 蓝图任务（设计态） | MinIO 同步下行，覆盖式 | 从 MinIO 拉最新，可被覆盖 |
| `cron/jobs.user.json` | 用户运行时加的任务 | **pod 本地私有盘（PVC）** | 原样保留，绝不被同步覆盖 |
| `cron/jobs.json` | 上两者合并结果，openclaw 实际加载 | pod 本地，每次启动重算 | 由合并器生成 |
| `cron/jobs-state.json` | 运行状态（下次触发/上次结果/错误数） | pod 本地，不进 MinIO | 原样保留 |

合并规则（pod 启动 / 收到同步事件时执行）：

```
1. 从 MinIO 拉取最新 jobs.blueprint.json（覆盖本地蓝图副本）
2. 读取本地 jobs.user.json（用户私有任务，不动）
3. 合并：blueprint ∪ user，按 job.id 去重
   - id 冲突时蓝图优先（蓝图是受管来源，用户不能覆盖蓝图任务）
4. 写出 cron/jobs.json → openclaw cron 系统加载
```

为区分来源，给 job 的 `id` 加前缀：蓝图任务 `bp:<name>`，用户任务 `usr:<name>`。合并器只清理"已不在蓝图里的 `bp:` 任务"，**绝不删 `usr:` 任务**。这样：

- 设计器删了某条蓝图任务 → 同步后 pod 上对应 `bp:` 任务消失
- 容器重启 → 蓝图重新拉取，但 `jobs.user.json` 在本地 PVC 上原样还在 → 合并后用户任务依旧存在

> 落地要点：pod 的 `cron/` 目录必须挂在**持久卷（PVC）**上，否则容器重启 `jobs.user.json` 和 `jobs-state.json` 都会丢。这与第一章"运行时状态留 pod 本地"是同一块盘。

### 6.4 新增 / 删除

按来源分流，两条路径互不干扰：

- **设计态新增蓝图任务**：设计器 UI 配置 → xsystem 校验（cron 可解析、`agentId` 存在、`at` 在未来）→ 分配 `bp:<id>` 追加进 MinIO 的 `jobs.blueprint.json` → 同步事件 → 所有相关 pod 重新合并并重载 cron。
- **运行态用户加任务**：用户在自己 OPT 的对话里说"每天 8 点提醒我交日报" → agent 用 openclaw 原生 `cron` 工具写本地 `jobs.user.json`（`usr:<id>`），**不经过 xsystem、不碰 MinIO**。
- **删除**：删谁的就改谁的来源文件——蓝图任务从 MinIO 蓝图删（影响所有 pod），用户任务从本地 `jobs.user.json` 删（只影响本 pod）。`at` 任务触发后自动删；删除不中断正在执行的那次，只取消后续调度。整点 cron 表达式 openclaw 自动加 stagger 错峰。

### 6.5 查看列表

按来源合并展示：

- **蓝图任务**：查 MySQL 清单（设计态定义：id/调度/目标 agent/描述），快、不依赖 pod 在线。
- **用户任务 + 运行态**（用户私有任务、下次触发、上次成败、错误数）：这些只在 pod 本地（`jobs.user.json` / `jobs-state.json`），不进 MinIO，需经对话通道 / 管理接口当场向 pod 拉取再合并返回。
