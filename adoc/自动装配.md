...
┌─────────────────────────────────────────────────────────────────┐
│                        xsystem（Java）                           │
│  · 接收 UI 配置，组装 OPT JSON，转发装配请求                      │
│  · MySQL：OPT 元数据 / 文件清单 / 版本 / pod 与用户的映射         │
│  · MinIO：workspace markdown 文件的唯一写入方                     │
│  · 基于一套 markdown 文件，调其他团队的服务为每个用户生成 OPT pod │
│  · 文件被 UI 修改后 → 写 MinIO → 发同步事件给对应 OPT pod         │
└──┬──────────────────────┬───────────────────────┬────────────────┘
   │ POST                  │ 读/写                  │ 调挂载服务
   │ /api/v1/opt/assemble  ▼                        │ (其他团队)
   │              ┌─────────────────┐               │
   │              │      MinIO      │               │
   │              │  workspace/*.md │ ← 一套文件=一个OPT蓝图
   │              └────────┬────────┘               │
   ▼                  ▲    │ ② 生成pod时拉取          │
┌──────────────────────┴──┴──┐                       │
│ OpenClaw · Assemble Agent  │                       │
│ （装配器，常驻一个）        │                       │
│  1. 校验输入               │                       │
│  2. DAG 校验 + 转换        │                       │
│  3. 渲染 workspace 文件集  │                       │
...
└────────────────────────────┘                       │
                                                     ▼
                              ┌──────────────────┐  ┌──────────────────┐
                              │ OPT pod: 张三     │  │ OPT pod: 李四     │ ……
                              │ (employee A)     │  │ (employee B)     │ 每用户一个
                              │ 本地盘 POSIX：    │  │ 本地盘 POSIX：    │
                              │ workspace/*.md   │  │ workspace/*.md   │ ← MinIO同套蓝图
                              │ registry.sqlite  │  │ registry.sqlite  │ ← 运行时,本地
                              │ memory/*.md      │  │ memory/*.md      │ ← agent产出,本地
                              └──────────────────┘  └──────────────────┘
                                  ▲ 由 xsystem 调其他团队服务创建并挂载
```

**三类角色，职责分开看：**

- **Assemble Agent（装配器）**：常驻一个，是个 openclaw 插件（`POST /api/v1/opt/assemble`）。它**只负责生成一套 OPT 所需的 markdown 文件并上传到 MinIO**，到此为止——不创建、不挂载 pod。它与 xsystem（收装配请求）和 MinIO（写文件）交互。
