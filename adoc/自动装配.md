...
### 6.2 定时任务是设计态资产，用户不可改

定时任务和 AGENTS.md / SKILL 一样，是**设计态统一定义的蓝图资产**：

- 只有设计器能增删改，经 xsystem 写 MinIO 蓝图。
- 用户在自己的 OPT 里**不能新增、不能修改、不能删除**定时任务。
- 一套蓝图下所有 pod（张三、李四…）跑同一份任务定义。

因此不存在"用户运行时加任务、重启被覆盖"的问题——`cron/jobs.json` 就是一个普通的蓝图配置文件，单向从 MinIO 下行，pod 本地副本只读。

openclaw 的双文件持久化（`src/cron/store.ts`）正好对应第一章"配置进 MinIO、运行时状态留本地"：

| 文件 | 内容 | 归属 | 重启行为 |
|------|------|------|---------|
| `cron/jobs.json` | 任务定义（schedule/payload/sessionTarget） | MinIO 蓝图真源，覆盖式下行 | 从 MinIO 拉最新，覆盖本地副本 |
| `cron/jobs-state.json` | 运行状态（下次触发/上次结果/错误数） | pod 本地私有盘（PVC），不进 MinIO | 原样保留 |

`jobs.json` 是声明式的，跟着蓝图走；`jobs-state.json` 是 openclaw 自己算出来的下次触发时间等运行态，留本地。两者边界与 SQLite / memory 一致。

> 落地要点：pod 的 `cron/` 目录挂在持久卷（PVC）上，让 `jobs-state.json` 跨重启保留触发进度。`jobs.json` 即使丢了也能从 MinIO 重新拉。

### 6.3 增删改：全部走设计态

增删改都只发生在设计器侧，复用 6.1「改 workspace 文件 → 写 MinIO → 同步 pod」机制，不另起通道：

```
设计器 UI 增/删/改任务
  → xsystem 校验（cron 表达式可解析、agentId 在 OPT 内存在、at 时间在未来）
  → 改 MinIO 的 cron/jobs.json + 更新 MySQL 任务清单
  → 为受影响的每个 pod 写一条 workspace.sync 待办
  → pod 到点拉取 → 从 MinIO 覆盖本地 jobs.json → 重载 cron 系统
```

- **新增**：分配 `jobId` 追加进 `jobs.json`，整点 cron 表达式 openclaw 自动加 stagger 错峰。
- **删除**：按 `jobId` 从 `jobs.json` 移除。`at` 任务触发后自动删；删除不中断正在执行的那次，只取消后续调度。
- **修改**：改调度或 payload，同样覆盖写 MinIO 后同步下行。

一套蓝图实例化成多个 pod 时，xsystem 按 MySQL 映射为**每个**相关 pod 各写一条同步待办，改一次蓝图同步到所有 pod。

### 6.4 查看列表

- **任务定义**（id/调度/目标 agent/描述）：查 MySQL 清单即可，快、不依赖 pod 在线，与 MinIO 蓝图一致。
- **运行态**（下次触发、上次成败、错误数）：在 `jobs-state.json`、只在 pod 本地、不进 MinIO，需经对话通道 / 管理接口当场向 pod 拉取再合并返回。
