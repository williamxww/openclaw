...
### 3.3 加载 runtime plugins `runtime-plugins`（536–542）

```ts
ensureRuntimePluginsLoaded({ config, workspaceDir, allowGatewaySubagentBinding });
```

确保 Gateway 启动计划（`startup.pluginIds`）中的 runtime 插件已加载进进程（channel、memory、hook、契约 provider 等）。**按本次 provider 补 harness** 在 3.6。

### 3.4 cron `before_agent_reply` hook（569–591）

仅 `trigger === "cron"` 时执行。若 hook 返回 `handled: true`，**整个 run 短路**，直接返回 hook 提供的 reply payload，不走模型。（`hookCtx` 组装在 556–568。）

### 3.5 Hook 模型选择（593–604）
...
### 3.7 解析 model manifest `model-resolution`（625–699）

```ts
const dynamicModelResolution = await resolveModelAsync(provider, modelId, agentDir, config, { skipPiDiscovery: true, ... });
// 必要时 ensureOpenClawModelsJson 后再 resolve（650–657）
const { model, authStorage, modelRegistry } = modelResolution;
const ctxInfo = resolvedRuntimeModel.ctxInfo;   // context window 大小
```

从 plugin 动态 hook / models.json 解析 model、context window、API 类型；可能按 harness 切换 `selectedPiRuntimeProvider`（658–672）。model 找不到则 `throw FailoverError`（675–682）。

### 3.8 构建 Auth Profile Store（701–886）

关键变量：

| 行号 | 变量 | 含义 |
|------|------|------|
| 705–721 | `authStore` / `attemptAuthProfileStore` | 主 auth store / attempt 级 scoped store |
| 870–880 | `profileCandidates` | 本次 run 可用的 auth profile id 列表（按优先级排序） |
| 885 | `profileIndex` | 当前使用的 profile 指针 |

### 3.9 初始化 Auth Profile（895–986）

```ts
const { advanceAuthProfile, initializeAuthProfile, maybeRefreshRuntimeAuthForAuthError, stopRuntimeAuthRefreshTimer }
  = createEmbeddedRunAuthController({ ... });
await initializeAuthProfile();   // 978–979（plugin harness 可能跳过）
```

`initializeAuthProfile` 完成首次 API key / OAuth token 获取，将结果写入 `apiKeyInfo`/`runtimeAuthState`。`startupStages.mark("auth")` 在 985。
...
#### ④ Context Overflow Compaction（1852–2163）
...
d. compaction 失败 / 次数耗尽 → return 上下文溢出错误 payload（2134–2162）
```

#### ④½ hook `before_agent_run` 阻断（2165–2196）

`promptErrorSource === "hook:before_agent_run"` 时直接 return 错误 payload，不走后续恢复。

#### ⑤ Codex App-Server Client Close 重试（2198–2211）
...
#### ⑦ assistantError 处理（2472–2661）

```
assistantError（模型调用成功但 assistant response 有错误）

├─ thinking level 降级 → thinkLevel = fallbackThinking; continue（2477–2482）
├─ auth 刷新成功 → continue（2530–2538）
├─ handleAssistantFailover()（2573–2613）
│    ├─ action === "retry"（rotate_profile / same_model_idle_timeout）
│    │    └─ continue（2615–2631）
│    └─ action === "throw" → throw FailoverError（2633–2660）
...
超时但最终助手 response 已完成（stopReason = "completed"/"end_turn"/"stop"）→ 使用已完成的 reply，不报超时错误。

#### ③ planning-only retry（2899–2940）
...
#### ⑥ compaction continuation retry（2983–3005）
...
#### ⑦ strict-agentic blocked（3013–3067）
...
if (incompleteTurnText) {
  await maybeMarkAuthProfileFailure(...);   // 3194–3198
...
```ts
await markAuthProfileSuccess({ store: profileFailureStore, profileId: lastProfileId, ... });   // 3237–3247
return {
